...
void dsi_set_key(dsi_context *ctx, unsigned char key[16]) {
	int i;
...
void dsi_set_ctr(dsi_context *ctx, unsigned char ctr[16]) {
	int i;
...
void dsi_init_ctr(dsi_context *ctx, unsigned char key[16], unsigned char ctr[12]) {
	dsi_set_key(ctx, key);
	dsi_set_ctr(ctx, ctr);
}

void dsi_crypt_ctr_block(dsi_context *ctx, unsigned char input[16], unsigned char output[16]) {
	int i;
...
void dsi_decrypt_ccm(
	dsi_context *ctx, unsigned char *input, unsigned char *output, unsigned int size, unsigned char *mac) {
...
void dsi_es_set_random_nonce(dsi_es_context *ctx) { ctx->randomnonce = 1; }
