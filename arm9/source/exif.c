...
static void put32(u8 *p, u32 v) { p[0] = v >> 24; p[1] = v >> 16; p[2] = v >> 8; p[3] = v & 0xFF; }
...
void buildExifApp1(u8 *rgb640x480, u8 **outBuf, int *outLen) {
	...
	// TIFF header
	tiff[0] = 'M'; tiff[1] = 'M'; tiff[2] = 0; tiff[3] = 42;
	put32(tiff + 4, P0);

	// ---- IFD0 ----
	u8 *p = tiff + P0;
	put16(p, 8); p += 2; // 8 entries

	u8 *extra = tiff + IFD0_EXTRA_START;
	int eoff = IFD0_EXTRA_START;

	memcpy(extra, "Nintendo", 9); // includes NUL
	putEntry(p, 0x010F, 2, 9, eoff); p += 12; extra += 9; eoff += 9;

	memcpy(extra, "NintendoDS", 11);
	putEntry(p, 0x0110, 2, 11, eoff); p += 12; extra += 11; eoff += 11;

	put32(extra, 72); put32(extra + 4, 1);
	putEntry(p, 0x011A, 5, 1, eoff); p += 12; extra += 8; eoff += 8;

	put32(extra, 72); put32(extra + 4, 1);
	putEntry(p, 0x011B, 5, 1, eoff); p += 12; extra += 8; eoff += 8;

	putEntry(p, 0x0128, 3, 1, 2u << 16); p += 12; // ResolutionUnit = inches

	memcpy(extra, "EINH", 5);
	putEntry(p, 0x0131, 2, 5, eoff); p += 12; extra += 5; eoff += 5;

	memcpy(extra, dt, 20);
	putEntry(p, 0x0132, 2, 20, eoff); p += 12; extra += 20; eoff += 20;

	putEntry(p, 0x8769, 4, 1, P1); p += 12; // ExifOffset -> SubIFD
	...
	putEntryRaw(p, 0x9000, 7, 4, (const u8 *)"0220", 4); p += 12; // ExifVersion

	memcpy(extra, dt, 20);
	putEntry(p, 0x9003, 2, 20, eoff); p += 12; extra += 20; eoff += 20; // DateTimeOriginal

	memcpy(extra, dt, 20);
	putEntry(p, 0x9004, 2, 20, eoff); p += 12; extra += 20; eoff += 20; // DateTimeDigitized

	{ u8 cc[4] = {1, 2, 3, 0}; putEntryRaw(p, 0x9101, 7, 4, cc, 4); p += 12; } // ComponentsConfiguration

	putEntryRaw(p, 0xA000, 7, 4, (const u8 *)"0100", 4); p += 12; // FlashPixVersion

	putEntry(p, 0xA001, 3, 1, 1u << 16); p += 12; // ColorSpace = sRGB
	putEntry(p, 0xA002, 4, 1, 640); p += 12;      // ExifImageWidth
	putEntry(p, 0xA003, 4, 1, 480); p += 12;      // ExifImageLength
	...
	putEntry(p, 0x0103, 3, 1, 6u << 16); p += 12; // Compression = old-style JPEG
	...
	putEntry(p, 0x0128, 3, 1, 2u << 16); p += 12; // ResolutionUnit
	putEntry(p, 0x0201, 4, 1, P3); p += 12;        // JPEGInterchangeFormat
	putEntry(p, 0x0202, 4, 1, thumbBuf.len); p += 12; // JPEGInterchangeFormatLength
